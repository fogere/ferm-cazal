...
export default function MapPage() {
  ...
  // Atténuation par segment — bug Eugénie 21/05/2026 Phase 2.
  // Formulaire d'ajout / modification d'une atténuation sur un water_stream sélectionné.
  // `editingId` non-null = modification d'une atténuation existante.
  const [streamAttenForm, setStreamAttenForm] = useState<{
    open:      boolean
    from:      number
    to:        number
    ratio:     number
        ...
        <div className="absolute inset-0 z-[2000] flex flex-col justify-end">
          ...
          <div className="relative bg-card rounded-t-3xl px-5 pt-5 pb-10 shadow-2xl max-h-[80vh] overflow-y-auto">
            ...
            {selected.type === 'water_stream' && (() => {
              ...
              const attenuations = selected.streamAttenuations ?? []
              const isSeasonal = selected.streamMode === 'seasonal'
              const months = selected.streamActiveMonths ?? []
              ...
              return (
                <div className="mb-4 space-y-3">
                              ...
                              <button
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    await updateDoc(doc(db, 'map_pins', selected.id), {
                                      streamAttenuations: next.length > 0 ? next : deleteField(),
                                      updatedAt: Date.now(),
                                      updatedBy: user.uid,
                                    })
                                    setSelected({
                                      ...selected,
                                      streamAttenuations: next.length > 0 ? next : undefined,
                                    })
                                  } finally { setActionBusy(false) }
                                }}
                                disabled={actionBusy}
                  ...
                  {!isTemp && streamAttenForm.open && points.length >= 2 && (
                    <div className="rounded-xl p-3 bg-sky/5 border border-sky/30 space-y-3">
                      <p className="text-xs font-semibold text-sky uppercase tracking-wider">
                        {streamAttenForm.editingId ? 'Modifier l\'atténuation' : 'Nouvelle atténuation'}
                      </p>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <label className="block">
                          ...
                          <select
                            value={streamAttenForm.to + 1}
                            onChange={e => {
                              const n = Math.max(streamAttenForm.from + 2, Math.min(points.length, parseInt(e.target.value, 10)))
                              setStreamAttenForm(f => ({ ...f, to: n - 1 }))
                            }}
                            className="w-full mt-1 px-2 py-1.5 rounded-lg border border-border bg-white text-xs"
                          >
                            {points.slice(streamAttenForm.from + 1).map((_, i) => (
                              <option key={i + streamAttenForm.from + 1} value={i + streamAttenForm.from + 2}>
                                Point {i + streamAttenForm.from + 2}
                              </option>
                            ))}
                          </select>
                        </label>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-sm font-bold text-sky">{fmtRatio(streamAttenForm.ratio)}</span>
                        </div>
                        <input
                          type="range"
                          min={0}
                          max={100}
                          step={5}
                          value={Math.round(streamAttenForm.ratio * 100)}
                          onChange={e => setStreamAttenForm(f => ({ ...f, ratio: parseInt(e.target.value, 10) / 100 }))}
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => setStreamAttenForm({ open: false, from: 0, to: 1, ratio: 0.1, editingId: null })}
                          disabled={actionBusy}
                        ...
                        <button
                          onClick={async () => {
                            ...
                            setActionBusy(true)
                            try {
                              const existing = attenuations
                              const newAtt = {
                                id:    streamAttenForm.editingId ?? (crypto.randomUUID?.() ?? `att-${Date.now()}-${Math.random().toString(36).slice(2)}`),
                                from:  streamAttenForm.from,
                                to:    streamAttenForm.to,
                                ratio: streamAttenForm.ratio,
                              }
                              const next = streamAttenForm.editingId
                                ? existing.map(a => a.id === streamAttenForm.editingId ? newAtt : a)
                                : [...existing, newAtt]
                              await updateDoc(doc(db, 'map_pins', selected.id), {
                                streamAttenuations: next,
                                updatedAt: Date.now(),
                                updatedBy: user.uid,
                              })
                              setSelected({ ...selected, streamAttenuations: next })
                              setStreamAttenForm({ open: false, from: 0, to: 1, ratio: 0.1, editingId: null })
                            } finally { setActionBusy(false) }
                          }}
                          disabled={actionBusy}
