...
## CHANTIER FLUIDITÉ / HYGIÈNE — 2 juillet 2026 (lot 2)

Deux couches de plus sorties en composants `React.memo` dans des fichiers dédiés.

- **`pages/map/layers/StreamLayer.tsx`** (couche cours d'eau) : avant, `getStreamSegments`
  était recalculé à CHAQUE render (inline). Désormais mémoïsé → recalcul seulement si
  `pins`/`anyModeActive` changent.
- **`pages/map/layers/OtherMembersLayer.tsx`** (positions + pointeurs des autres membres) :
  deux blocs `.map` inline qui se recréaient à chaque render (dont le tick `now`) → un seul
  composant mémoïsé sur `users`/`selfUid`/`now`. Z-order préservé via `zIndexOffset`.
- `makeUserLocationIcon` + `makePointerIcon` déplacés dans `services/map/pinIcons.ts`.
- **Map.tsx : 6203 → 6117 lignes** (−232 depuis le début de session). tsc + build verts,
  comportement identique. Déployé le-cazal.web.app.

### Suite du chantier (prochains lots)
- Continuer à isoler les couches encore inline/lourdes (labels enclos, aperçu édition
  clôture) et migrer les `useMemo` de couches (landPlotLayers/fenceLayers/landPlotLabels)
  vers des composants fichiers → Map.tsx sous 5K.
- Puis s'attaquer à la surface de re-render (94 `useState`) : regrouper les états de
  mode UI pour qu'ils ne re-render plus toute la carte. C'est là le gros du gain fluidité.
- Rafraîchir `ARCHITECTURE.md` (dit encore Map = 4400).

---

## CARTE — proxy de tuiles Cloudflare (2 juillet 2026)

Problème Nils : **carrés noirs + carte qui recharge en boucle** au déplacement. Cause :
on tapait `data.geopf.fr` en **direct depuis chaque téléphone** → l'IGN nous rate-limite
(fair-use public) → tuiles en erreur (carré noir) + jamais mises en cache → redemandées
en boucle.

**Fix : proxy + cache edge Cloudflare Worker** (`tile-proxy/`, gratuit, 100k req/j).
- `tile-proxy/index.js` : forwarde la query WMTS vers l'IGN, met chaque tuile en cache
  edge 30 j (`caches.default`), CORS + `Cache-Control: max-age=2592000 immutable`,
  allowlist stricte (ortho / plan / parcelles), ne cache JAMAIS une erreur.
- Déployé par Nils sur son compte Cloudflare (dashboard, pas wrangler — CLI plantait
  `libuv` sur Windows). URL : **`https://ferme-tiles.ferme-nilslamber.workers.dev`**.
- **Client rebranché** sur le worker : `Map.tsx` (aérien/plan/parcelles), `MapPicker.tsx`,
  `services/map/precacheTiles.ts` (carte hors-ligne), routes cache `sw.ts` (hostname), et
  CSP `firebase.json` (img-src + connect-src). Déployé le-cazal.web.app.
- Vérifié en prod : la tuile revient `X-Tile-Cache: HIT` → servie par Cloudflare, l'IGN
  n'est plus sollicité. Fin attendue des carrés noirs / rechargement en boucle ; zones
  déjà vues instantanées.
- ⚠️ L'ancien worker `worker/` (cron FCM, projet test farm-ed787) est **distinct** et
