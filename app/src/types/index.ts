...
export interface MapPin {
  ...
  // ── land_plot : référencement officiel / administratif ──
  // Demande Nils 27/05/2026 : import des fiches papier des 7 terrains.
  // Chaque fiche donne la référence cadastrale, l'identifiant PAC, la surface
  // officielle, et le propriétaire (parcelles parfois en fermage chez voisin).
  // Tous optionnels — un land_plot dessiné à main levée sur la carte peut
  // exister sans ces données.
  // Pour un parc à parcelle unique (ex: Fond Rouge bas B988), ces champs scalaires
  // suffisent. Pour un parc qui REGROUPE plusieurs parcelles cadastrales
  // (ex: Fontrouge = 11 parcelles B952..B975 chez 6 propriétaires différents),
  // utiliser le tableau `parcels` ci-dessous — les champs scalaires restent
  // optionnels et peuvent servir à exposer la parcelle "principale" pour l'UI.
  cadastralRef?: string                 // ex: "B988"
  pacIlot?:      string                 // numéro d'îlot PAC, ex: "53"
  pacYear?:      number                 // année de déclaration PAC, ex: 2023
  // 'declared' = mis à la PAC pour l'année N (réversible). 'engaged' = engagement
  // pluriannuel PAC (ex: MAEC, conversion bio) — la parcelle est bloquée
  // contractuellement, ne pas la sortir du système sans vérifier l'engagement.
  pacStatus?:    'declared' | 'engaged'
  surfaceM2?:    number                 // surface officielle (m²) — si vide, calculer du polygone
  landowner?:    string                 // ex: "Sophie Audinos" ; vide = nous
  leaseType?:    'owned' | 'fermage' | 'gracious' | 'unknown'
  leaseNote?:    string                 // contexte du fermage (mois de signature, conditions…)

  // Détail par parcelle cadastrale, quand un parc en regroupe plusieurs.
  // Chaque entrée garde sa propre référence cadastrale, surface et statut
  // foncier — un seul parc peut mélanger des parcelles à nous, en fermage,
  // et à des propriétaires différents (cas typique de Fontrouge).
  parcels?: Array<{
    id:            string               // uuid local — clé React, facilite l'édition
    cadastralRef:  string               // ex: "B957"
    surfaceM2?:    number
    landowner?:    string
    leaseType?:    'owned' | 'fermage' | 'gracious' | 'unknown'
    leaseNote?:    string               // ex: "Accord 6/01/25"
    note?:         string               // libre (lieu-dit, contexte spécifique à cette parcelle)
  }>

  // ── water_stream : journal d'observations ──
  // Demande Nils 27/05/2026 : les fiches papier consignent depuis 2024 les
  // observations de débit ("beaucoup d'eau le 17/12/2024", "à sec le 11/12/2025").
  // Tableau inline (volume attendu < 100 entrées sur 5 ans). À déplacer en
  // sous-collection si on dépasse les 500 entrées par stream.
  streamObservations?: Array<{
    id:    string                       // uuid local — clé React
    date:  number                       // timestamp ms de l'observation
    level: 'dry' | 'low' | 'normal' | 'high'
    note?: string                       // contexte ("après orage", "fin de sécheresse"…)
    observedBy: string                  // uid (ou displayName si import historique)
  }>
...
// Feature Saisie/Enrichissement : la famille répond en groupe (sur 1 compte) à
// un ensemble de questions générées depuis les fiches papier des terrains
// (fichier/*.docx/odt). Les questions sont statiques (app/src/data/parcelQuestions.ts).
// Les réponses sont stockées dans Firestore (collection `parcel_answers`) pour
// que le script d'import les reprenne dans une 2e passe.
//
// Décisions UX 2026-05-27 :
//   - 1 compte commun pour répondre → pas de suivi par utilisateur sur les réponses
//   - n'importe qui peut valider — le bouton "valider" finalise la réponse
//   - mix de types de question (texte, choix, dates, placement carte pin/polygone…)

export type ParcelQuestionType =
  | 'text'             // texte libre (réponse courte)
  | 'long_text'        // texte libre paragraphe (réponse longue)
  | 'single_choice'    // 1 choix parmi options[]
  | 'multi_choice'     // 0..N choix parmi options[]
  | 'yes_no'           // oui / non / je sais pas
  | 'date'             // une date YYYY-MM-DD
  | 'animals_pick'     // sélection d'animaux existants depuis le cheptel
  | 'plot_pick'        // sélection d'un land_plot existant
  | 'pin_on_map'       // tap sur la carte → { lat, lng }
  | 'polygon_on_map'   // dessin d'un polygone → Array<{lat,lng}>

export type ParcelQuestionCategory =
  | 'plot_match'        // identifier un plot dans le cheptel existant
  | 'plot_create'       // créer un nouveau plot (coordonnées + nom + cadastre)
  | 'water_match'       // identifier ou créer un point d'eau / ruisseau
  | 'session_animals'   // confirmer quels animaux dans une session
  | 'session_dates'     // confirmer / corriger des dates
  | 'owner_confirm'     // confirmer un type de fermage / propriétaire
  | 'narrative'         // confirmer / enrichir une note narrative
  | 'other'

export interface ParcelQuestion {
  id:           string                          // slug stable, sert d'id de doc dans Firestore
  sourceDoc:    string                          // ex: "fichier/Fond rouge bas 988.odt"
  sourceQuote:  string                          // citation EXACTE du doc (la famille la reconnaîtra)
  category:     ParcelQuestionCategory
  question:     string                          // la question posée à la famille
  context?:     string                          // contexte additionnel (pourquoi on demande, ce qu'on en fera)
  questionType: ParcelQuestionType
  options?:     Array<{ id: string; label: string; hint?: string }>  // pour single/multi_choice
  // Indicateurs visuels facultatifs : si présents, l'UI peut centrer la carte
  // ou afficher la fiche animal en référence pour aider la famille à répondre.
  relatedPinId?:      string
  relatedAnimalIds?:  string[]
  // Ordre d'affichage (regroupé par doc puis par order croissant)
  order:        number
}

// La valeur de la réponse change selon questionType. Stockée dans Firestore en
// `value` (any pour simplicité de stockage Firestore). Le helper côté client la
// re-type en lecture.
export interface ParcelAnswer {
  id:           string         // = ParcelQuestion.id
  value:        unknown        // dépend de questionType
  note?:        string         // commentaire libre (la famille peut expliquer son raisonnement)
  answeredBy:   string         // uid (audit trail — 1 seul compte mais on garde la trace)
  answeredByName?: string
  answeredAt:   number
  // Marqueur "ignore cette question" : la famille a décidé qu'on ne peut/veut pas y répondre.
  // L'import ne traitera pas cette question.
  skipped?:     boolean
  // Une fois la réponse appliquée par le script d'import, on note ici la date
  // pour que le bouton "valider" devienne grisé "déjà traité".
  importedAt?:  number
}
